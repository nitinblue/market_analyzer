...
First-time OAuth setup::
    ...
    schwab.auth.easy_client(
        ...
        token_path=str(Path.home() / ".income_desk" / "schwab_token.json"),
    )
...
if TYPE_CHECKING:
    ...
    from income_desk.broker.schwab.metrics import SchwabMetrics
...
def connect_schwab(
    ...
    3. ``~/.income_desk/broker.yaml`` under the ``schwab`` key

    Token file resolved in order:
    1. Explicit ``token_path`` argument
    2. ``~/.income_desk/schwab_token.json``
