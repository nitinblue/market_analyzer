...
from income_desk.broker.base import BrokerSession
...
class TastyTradeBrokerSession(BrokerSession):
    ...
    def _find_config_file(self) -> Path | None:
        ...
        2. ``~/.income_desk/broker.yaml``  — written by the setup wizard
        3. ``~/.income_desk/<config_path>``
        4. Package-relative paths (dev/project-root usage)
        """
        candidates = [
            Path(self._config_path),
            # Setup wizard writes here
            Path.home() / ".income_desk" / "broker.yaml",
            Path.home() / ".income_desk" / self._config_path,
