...
> Type: INTAKE | Last reviewed: 2026-03-29 | Staleness: AGING

## Active Items

| Key | Item | Added | Last Actioned | Status | Assignee | Next Action | Blockers | Delivered To |
|-----|------|-------|---------------|--------|----------|-------------|----------|--------------|
| BUG-001 | BAC/GBTC equity 100x multiplier in regression | 2026-03-26 | 2026-03-26 | OPEN | Claude | Debug regression validator equity multiplier logic | — | — |
...
| BUG-003 | Ranked trades show POP 1% for all India trades — clearly wrong. SBIN IC with 0.55 score but 1% POP makes no sense. POP calculation broken for India market | 2026-03-29 | 2026-03-29 | OPEN | Claude | Debug POP calculation for India tickers — likely missing data due to BUG-002 (yfinance 404) causing fallback to garbage defaults | — | — |
| BUG-004 | India credits nonsensical — SBIN IC credit INR 0.63 with max risk INR 152,925 (0.0004% return). ICICIBANK credit INR 0.15. These are not real tradeable numbers | 2026-03-29 | 2026-03-29 | OPEN | Claude | Credit/risk calculation broken for India. Likely same root cause as BUG-002/003 — no real option chain data, falling back to bad estimates | — | — |
| BUG-005 | Validate trade passes ALL gates on garbage data — SBIN with INR 0.63 credit, 0.0 current_price, DTE=1, regime_id=1 (actual is R2). Gates are rubber-stamping bad input instead of catching it. current_price=0 alone should FAIL. Detail column mostly empty | 2026-03-29 | 2026-03-29 | OPEN | Claude | Gates need minimum data quality checks: current_price > 0, credit makes sense vs risk, regime_id matches actual detection. Detail column must explain WHY each gate passed/failed | — | — |
| BUG-006 | Sizer returns 0 contracts — pop_pct 0.9978 (99.78%) is nonsensical input from ranking, max_loss INR 152,925 exceeds capital INR 50,000, wing_width 5.0 is US default not India. Kelly correctly returns 0 because the numbers are garbage, but the workflow should have halted earlier | 2026-03-29 | 2026-03-29 | OPEN | Claude | Sizer should detect and reject impossible inputs: max_loss > capital, pop_pct > 99%, wing_width mismatched for market. Root cause is upstream (BUG-002/003/004) | — | — |
| BUG-007 | Price trade: SBIN IC wing width only 5 points (975/970/1020/1025) on a INR 1000 stock — too narrow. IV shows 0.5-0.6% which is wrong (should be ~20-30%). Net credit INR 0.53 for an IC is untradeable. Strikes are from fallback estimation (price*0.97/0.93), not from actual chain | 2026-03-29 | 2026-03-29 | OPEN | Claude | India strike selection needs ATR-based wing width, not US $5 default. IV values are likely Dhan raw (needs /100 per auto-memory). Price workflow should refuse to quote if credit < minimum viable threshold | — | — |
