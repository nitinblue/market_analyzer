...
| 6 | Chain-repriced credits (not estimated) | 10% | ranking shows "chain_repriced=True", credit >1 INR | PASS (2026-03-30: single-pass PricingService, 33 trades assessed via chain, 7/7 regression validated) |
...
| 16 | No simulated data in output | 5% | No "DEMO-", no "simulated", no FRED errors | PARTIAL (demo positions expected when no broker positions; no fake pricing data) |
...
| POP DTE bug | 7, 13 | +15% |
| Wing width fix | 9 | +5% |
| Chain repricing all tickers | 6, 8 | +9% (6 DONE, 8 partial) |
