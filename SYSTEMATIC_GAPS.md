...
| TA7 | Context | Tradeable instruments in daily context | **DONE** | `MarketContext.tradeable: InstrumentAvailability` — options/stocks/futures availability + viable strategies per regime. India weekly expiry detection (Thu=NIFTY, Wed=BANKNIFTY, Tue=FINNIFTY). Auto-populated by `context.assess()`. eTrading: display in daily pre-market dashboard, gate order types by instrument availability. | `generate_futures_report(ticker, spot, futures, iv, regime)` — combines all above. eTrading: call for futures desk dashboard. | `decide_wheel_action(position, regime)` — eTrading passes `WheelPosition` state, MA returns `WheelDecision` with next action + trade params. States: IDLE→PUT_OPEN→ASSIGNED→CALL_OPEN→CALLED_AWAY. Regime-aware: R4=pause wheel. eTrading: build state machine (persistence, transitions, execution). Call `decide_wheel_action()` on every state change. | `check_rebalance(current, target, value, drift_threshold)` — buy/sell actions when drift >5%. CLI `rebalance`. eTrading: run quarterly, execute actions. | `challenge/trader_stocks.py` — 8-step flow: context → universe → data → screen → picks → sectors → portfolio construction → exit rules. `--market US|India` switch. |
...
| 2026-03-15 | CD1-CD8 + WH1 + FT1-FT7: Capital deployment, LEAP vs stock, wheel, futures analysis | +0 | 1331 |
